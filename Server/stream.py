...
TCP_CMD = "ffmpeg -re -fflags +genpts -i stream.mp4 -an -c:v libx264 -preset ultrafast -f mpegts tcp://127.0.0.1:3333?listen"
UDP_CMD = "ffmpeg -re -fflags +genpts -i stream.mp4 -an -c:v libx264 -preset ultrafast -f mpegts -flush_packets 0 udp://127.0.0.1:3333?pkt_size=1024"
RTSP_CMD ="ffmpeg -re -fflags +genpts -i stream.mp4 -an -c:v libx264 -preset ultrafast -f rtsp rtsp://localhost:8554/mystream"
RTMP_CMD ="ffmpeg -re -fflags +genpts -i stream.mp4 -an -c:v libx264 -preset ultrafast -f flv -listen 1 rtmp://127.0.0.1:3333"
SRT_CMD = "ffmpeg -re -fflags +genpts -i stream.mp4 -an -c:v libx264 -preset ultrafast -f mpegts srt://127.0.0.1:3333?mode=listener"
